...
img_1 = cv.imread("image1.jpg")
x, y, _ = img_1.shape
img_1 = cv.resize( img_1, ( int(y/4), int(x/4)))
...
for i in range(len(face_L)):
    ...
    cv.rectangle( img_1, (x1,y1), (x2,y2), (0, 255, 255), 2)
    ...
    for j in range(len(face_encoding)):
        if Conditions[j] == True :
            if R_distance[j] < 0.5:
                cv.putText( img_1, person[j], (x1,y1), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                print(person[j])
